...
def main():
    ...
    model = VideoModel(
        ...
        add_fc=args.add_fc,
        fc_dim=args.fc_dim,
        dropout_i=0.0, #args.dropout_i
        dropout_v=0.0, #args.dropout_v
    ...
    # ## szf
    # save_ckpt_num = 10 # 10
    # save_interval = (args.epochs + 1) // save_ckpt_num
    # print('save ckpt every {} epochs'.format(save_interval))
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        loss_c, attn_epoch_source= train(
            num_class, source_loader, model, criterion,
            optimizer, epoch, train_file, train_short_file,
            alpha, beta, gamma, mu)
        ...
        #                 'best_prec1': best_prec1,
        #                 'prec1': prec1,
        #             }, is_best, path_exp,filename=filename)
...
# szf
def train(num_class, source_loader, model, criterion,
          optimizer, epoch, log, log_short, alpha, beta,
          gamma, mu):
    ...
    data_loader = enumerate(source_loader)
    ...
    ## szf modified
    for i, (source_data, source_label) in data_loader:
        ...
        #====== forward pass data ======#
        # szf
        # attn_source, out_source, out_source_2,  feat_source = model(source_data, beta, mu, is_train=True, reverse=False)
        attn_source, out_source, out_source_2,  feat_source = model(source_data, is_train=True)

        # ignore dummy tensors
        attn_source, out_source, out_source_2, feat_source = removeDummy(
            attn_source, out_source, out_source_2, 
    ...
    if args.tensorboard:
        ...
        feat_all_display_new = torch.cat(
            (feat_source_display_new), 0)
        label_all_display_new = torch.cat(
            (label_source_display_new), 0)
...
def validate(val_loader, model, criterion, num_class, epoch, log):
    ...
    for i, (val_data, val_label) in enumerate(val_loader):
        ...
        with torch.no_grad():
            ...
            # # compute output
            # attn_val, out_val, out_val_2, feat_val = model(
            #     val_data,
            #     [0] * len(args.beta),
            #     0,
            #     is_train=False,
            ...
            # ignore dummy tensors
            attn_val, out_val, out_val_2,  feat_val = removeDummy(
                attn_val, out_val, out_val_2, feat_val,
                batch_val_ori)
...
def removeDummy(attn, out_1, out_2, feat, batch_size):
